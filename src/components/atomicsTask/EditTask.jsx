...
console.log(JSON.stringify(data))
    await instance.post(apiDB.editTask+`?entry=${data.task}&contractor=${data.contractor}&date=${data.date}&duration=${data.duration}&billable=${data.billable}&project=${data.project}&product=${data.product}&activity=${data.activity}&category=${data.category}&description=${data.description}`)
alert("recibido")
}
       useEffect (()=>{ 
        getData()
      ...
      <div>
          ...
          <select {...register("task")}>
              {task?.map((option, index) => (
                  <option key={index} value= {option.entry_number}>
            {option.entry_number}  {option.description}
                  </option>
              ))}
          </select>
